...
def main():
    ...
    # Insert GitHub user name and email into library.json using simple replacement
    try:
        name = subprocess.check_output(['git', 'config', 'user.name'], encoding='utf-8').strip()
        email = subprocess.check_output(['git', 'config', 'user.email'], encoding='utf-8').strip()
        lib_path = 'library.json'
        if os.path.exists(lib_path):
            with open(lib_path, 'r', encoding='utf-8') as f:
                content = f.read()
            if '"name": "author_name"' in content and '"email": "author_email"' in content:
                content = content.replace('"name": "author_name"', f'"name": "{name}"')
                content = content.replace('"email": "author_email"', f'"email": "{email}"')
                with open(lib_path, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"Updated author name and email in {lib_path}")
            else:
                print(f"Skipped updating author info in {lib_path} (placeholders not found)")
    except Exception as e:
        print(f"Failed to update author info in library.json: {e}")
